...
namespace Cryptographer
{
    class Program
    {
        ...
        {
            //InitDatabase();
            TestDecryptor(TestEncryptor());
        }

        static string TestEncryptor()
        {
        ...
        static void TestDecryptor(string encryptedKey)
        {
            var publicKey_RSA = "<?xml version=\"1.0\" encoding=\"utf-16\"?><RSAParameters xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\"><Exponent>AQAB</Exponent><Modulus></Modulus></RSAParameters>";
            var key_AES = "T3y9J2fWnYhvVuJw+DsiiLgoOGlVMYUsW1qxSuATWGA=";
            var iv_AES = "QWqheHNK08BTYkUZR2/FPQ==";

            var decryptor = new Decryptor(publicKey_RSA, key_AES, iv_AES);



            var decryptedData = decryptor.DecryptAES<JsonElement>(encryptedKey, decryptor.Key_AES, decryptor.IV_AES);

            if(decryptedData.TryGetProperty("Data", out JsonElement keyData) && decryptedData.TryGetProperty("Hash", out JsonElement keyHash))
            {
                var jsonData = JsonSerializer.Serialize<object>(keyData);
                Console.WriteLine("Serialized keyData is:");
                Console.WriteLine(jsonData);
                Console.WriteLine();

                Console.WriteLine("keyHash is:");
                Console.WriteLine(keyHash.ToString());
                Console.WriteLine();

                var badHash = "badHash";
                var badHashBase64 = "YmFkSGFzaA==";
                
                var verifyedValidHash = decryptor.VerifyHashRSA(keyData, keyHash.ToString(), decryptor.PublicKey_RSA);
                var verifyedInvalidHash = decryptor.VerifyHashRSA(keyData, badHash, decryptor.PublicKey_RSA);
                var verifyedInvalidHashBase64 = decryptor.VerifyHashRSA(keyData, badHashBase64, decryptor.PublicKey_RSA);

                Console.WriteLine($"Verify hash:");
                Console.WriteLine($"Valid hash:\tInvalid hash:\tInvalid hash base64:");
                Console.WriteLine($"{verifyedValidHash}\t\t{verifyedInvalidHash}\t\t{verifyedInvalidHashBase64}");
                Console.WriteLine();
                Console.WriteLine();
                Console.WriteLine();
            }
